...
- `charts/`: Thư mục chứa các `dependencies` cụ thể của `Helm Chart`. Các `dependencies` này sẽ được cài đặt cùng với `Helm Chart` chính.
...
1. Đầu tiên cần tạo tên của `Helm Chart`:
```bash
helm create <tên Helm Chart>
```
SAU KHI CHẠY LỆNH TRÊN, MỘT THƯ MỤC MỚI SẼ ĐƯỢC TẠO RA VỚI TÊN LÀ `<tên Helm Chart>`.

TIẾN HÀNH XÓA CÁC `FILE` TRONG `TEMPLATES/` VÀ XÓA HẾT CÁC NỘI DUNG TRONG `values.yml` ĐỂ BẮT ĐẦU TẠO `Helm Chart` MỚI.

2. Tạo các `file template` chung trong thư mục `templates/` (Ví dụ như `deployment.yaml`, `service.yaml`, `configmap.yaml`, `ingress.yaml`, `secret.yaml`,...). Lưu ý các `file template` sẽ là khung của `Kubernetes manifest` cho ứng dụng nên cần phải mang tính chất chung và tái sử dụng.
  > Ví dụ file `service.yaml`:
  ```yaml
...
5. `helm install <name> <chart>`: Cài đặt `chart` có tên `<name>` từ `repository` vào `Helm`.
...
8. `helm uninstall <name>`: Gỡ cài đặt `chart` có tên `<name>` từ `repository` trong `Helm`.
