...
>`Spring Cloud Gateway` là một dự án của `Webflux` cho nên khi xấy dựng `Gateway Server` chúng ta sẽ sử dụng các dependence hỗ trợ `Reactive Programming` để xử lý các request từ bên ngoài và chuyển tiếp đến các dịch vụ khác trong hệ thống.
  ...
  public GlobalFilter postGlobalFilter() { // Tạo ra một GlobalFilter để xử lý logic
    return (exchange, chain) -> { // Lambda expression, xử lý logic
      return chain.filter(exchange).then(Mono.fromRunnable(() -> { // Xử lý logic bất đồng bộ
        ...
        if (!(exchange.getResponse().getHeaders().containsKey(filterUtility.CORRELATION_ID))) { // Kiểm tra xem correlation-id đã tồn tại trong headers của response hay chưa, tránh trường hợp khi retry request sẽ tạo ra nhiều correlation-id
...
## 3. Sử dụng `Consul` làm `Service Discovery` cho `Microservices` và `Gateway Server` thay vì `Eureka Server`
### 3.1. Giới thiệu
`Consul` là một công cụ `Service Discovery` và `Configuration Management` mã nguồn mở được phát triển bởi `HashiCorp`. Nó cung cấp một cách đơn giản và hiệu quả để quản lý các dịch vụ trong một hệ thống phân tán, đặc biệt là trong môi trường `Microservices`.
`Consul` cung cấp các tính năng chính sau:
- `Service Discovery`: Giúp các dịch vụ trong hệ thống có thể tìm thấy và giao tiếp với nhau một cách dễ dàng.
- `Health Checking`: Giúp kiểm tra tình trạng hoạt động của các dịch vụ và đảm bảo rằng chỉ những dịch vụ hoạt động tốt mới được sử dụng.
- `Key/Value Store`: Cung cấp một kho lưu trữ dữ liệu dạng key-value để lưu trữ cấu hình và thông tin khác.
- `Multi-Datacenter`: Hỗ trợ triển khai và quản lý dịch vụ trên nhiều trung tâm dữ liệu khác nhau.
### 3.2. Cài đặt `Consul`
Để cài đặt `Consul`, chúng ta có thể sử dụng `Docker` để chạy `Consul Server` một cách nhanh chóng và dễ dàng. Dưới đây là các bước để cài đặt `Consul` bằng `Docker`
```yml
services:
  consult:
    image: consul:1.15.4
    container_name: consult_server
    ports:
      - "8500:8500" # HTTP API and Web UI
      - "8600:8600/udp" # DNS interface for service discovery
    command: "agent -server -ui -node=consult-server -bootstrap-expect=1 -client=0.0.0.0" # Cấu hình Consul ở chế độ server với giao diện web và chấp nhận kết nối từ mọi địa chỉ IP 
    environment:
      CONSUL_BIND_INTERFACE: eth0 # Đặt giao diện mạng mà Consul sẽ sử dụng để liên lạc với các nút khác
    volumes:
...
### 3.3. Cấu hình `Microservices` và `Gateway Server` kết nối tới `Consul`
Để cấu hình `Microservices` và `Gateway Server` kết nối tới `Consul`, chúng ta cần thêm các `dependency` sau vào `pom.xml` của từng `service`.
- `Spring Cloud Consul Discovery`: là thư viện dùng để đăng ký `service` lên `Consul Server`.
- `Spring Cloud Consul Config`: là thư viện dùng để cấu hình `service` từ `Consul Server` (nếu cần), mặc định đang sử dụng `Config Server` nên bỏ qua.
- `Spring Boot Actuator`: là thư viện dùng để quản lý và giám sát `service`.
- `spring-cloud-starter-gateway-server-webflux`: là thư viện dùng để xây dựng `Gateway Server` với `Spring Cloud Gateway` và `Webflux` (nếu là `Gateway Server`) (Phiên bản mới nên dùng thay thế cho `spring-cloud-starter-gateway`).
>pom.xml
```xml
<dependency>
    <groupId>org.springframework.cloud</groupId>
...
### 3.4. Cấu hình kết nối tới `Consul Server`.
Đầu tiên thêm `@EnableDiscoveryClient` vào `Application.java` của từng `service` để kích hoạt `Service Discovery` với `Consul`.
>Application.java
```java
@SpringBootApplication
@EnableDiscoveryClient // Kích hoạt Service Discovery với Consul
...
Tiếp theo, cấu hình kết nối tới `Consul Server` trong `application.yml` của từng `service`.
>application.yml
```yml
server:
  port: 8088
    ...
    gateway:
      server:
        webflux:
          discovery:
            locator:
              enabled: false # Tắt tự động định tuyến dựa trên service discovery (Khi sử dụng Consul)
              lower-case-service-id: true # Đặt service ID thành chữ thường khi định tuyến
    consul:
      host: localhost # Địa chỉ của Consul agent (localhost nếu chạy cục bộ)
      port: 8500 # Cổng của Consul agent
      enabled: true # Kích hoạt tích hợp với Consul
      discovery:
        prefer-ip-address: true # Ưu tiên sử dụng địa chỉ IP thay vì hostname
        register: true # Đăng ký service với Consul
        instance-id: ${spring.application.name}-${server.port}-${random.int[1,99]} # Định danh duy nhất cho instance
        health-check-path: /actuator/health # Đường dẫn health check
        service-name: ${spring.application.name} # Tên service khi đăng ký với Consul
        health-check-interval: 10s # Khoảng thời gian giữa các lần health check
        health-check-critical-timeout: 1m # Thời gian timeout để đánh dấu service là không khỏe mạnh
        health-check-timeout: 5s # Thời gian chờ cho mỗi lần health check
        hostname: localhost # Tên host để đăng ký với Consul
        tags: # Thêm thẻ để phân loại service trong Consul
          - gateway
          - api-gateway
          - version=1.0.0
        fail-fast: true # Nếu không thể kết nối với Consul, ứng dụng sẽ không khởi động
      config:
        enabled: false
...
management:
  health:
    consult:
      enabled: true # Kích hoạt health check với Consul
    readinessstate:
      enabled: true # Kích hoạt readiness state
    livenessstate:
      enabled: true # Kích hoạt liveness state
  endpoints:
    web:
      exposure:
        include: "*" # Cho phép hiển thị tất cả các endpoint
  endpoint:
    gateway:
      access: read_only # Cho phép truy cập endpoint /actuator/gateway
  info:
    env:
      enabled: true # Cho phép dùng endpoint /actuator/info để xem thông tin về service
