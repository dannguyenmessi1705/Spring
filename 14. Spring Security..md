...
```
## 4. Cấu hình CORS (Cross-Origin Resource Sharing) trong Spring Security
...
Để cho phép các trang web khác truy cập vào tài nguyên của trang web hiện tại, chúng ta cần cấu hình `CORS` trong `Spring Security` từ `WebMvcConfigurer`.
...
## 5. Cấu hình `SecurityFilterChain` trong `Spring Security`
...
> Cần phải để `SessionCreationPolicy.STATELESS` nếu không sử dụng `Session` trong `RESTAPI`.
```java
...
...
### 5.1. Cấu hình để `H2-DATABASE` hoạt động với Spring Security
