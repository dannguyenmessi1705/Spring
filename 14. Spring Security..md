...
...
	...
	public ResponseEntity<?> auth(@RequestBody Login login) { // Phương thức xác thực User từ Database bên ngoài
		authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(login.getUsername(), login.getPassword())); // sau khi gọi phương thức này, sẽ gọi phương thức loadUserByUsername(username) của CustomUserDetails để xác thực User từ Database bên ngoài
