...
public class SecurityConfig {
	...
	@Bean
	public UserDetailsService userDetailsService(DataSource dataSource) { // Bean này trả về một đối tượng UserDetailsService, dùng để tạo ra một user trong H2-Database
		...
		return jdbcUserDetailsManager; // Trả về một đối tượng UserDetailsService
	}
}
```

## 5. Cấu hình `Spring Security` tạo, xác thực `User` trong `Database` bên ngoài (MYSQL) (RESTAPI: `Basic Authentication`)
Để thay vì lưu trữ `User` trong `H2-Database`, chúng ta có thể lưu trữ `User` trong `Database` bên ngoài như `MYSQL`. Sử dụng `JDBC`, `JPA` để kết nối với `MYSQL`.

Cấu hình kết nối với `MYSQL` xem ở `08. JPA, Hibernate, JDBC.md`.

### 5.1. Cấu hình `CustomUserDetailsService` để xác thực `User` trong `MYSQL`
Mặc định, `Spring Security` sử dụng `UserDetailsService` để xác thực `User` trong `In-Memory` hoặc `Database` của chính ứng dụng. Để xác thực `User` trong `Database` bên ngoài, chúng ta cần tạo một class `CustomUserDetailsService` và implement `UserDetailsService`. Và gán cho nó là `@Service`.
...
@Service
public class CustomUserDetails implements UserDetailsService { // Kế thừa UserDetailsService để xác thực User trong Database bên ngoài

	private final UserRepository userRepository; // Inject UserRepository để truy vấn User trong Database
	...
	@Override
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException { // Phương thức xác thực User theo username. Mặc định khi gọi 
	// authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(username, password)) 
	// sẽ gọi phương thức này loadUserByUsername(username)
		Users user = userRepository.findByUsername(username); // Tìm User trong Database theo username
		if (user == null) { // Nếu không tìm thấy User
			throw new UsernameNotFoundException("User not found"); // Ném ra ngoại lệ UsernameNotFoundException
		}
		return new MyUserPrincipal(user); // Trả về một đối tượng MyUserPrincipal với User tìm thấy
	}
	
}

// Custom UserDetails
class MyUserPrincipal implements UserDetails { // Kế thừa UserDetails để tạo ra một UserDetails từ User

	private final Users user; // Inject Users để lấy thông tin User
	...
	@Override
	public Collection<? extends GrantedAuthority> getAuthorities() { // Lấy danh sách các quyền của User (Role) 
		return user.getRoles().stream() 
				.map(role -> "ROLE_" + role) // Map role thành ROLE_role
				.map(SimpleGrantedAuthority::new) // Map thành SimpleGrantedAuthority
				.collect(Collectors.toList()); // Trả về danh sách các quyền của User
				// Có thể để rỗng, new ArrayList<>() nếu không sử dụng quyền
	}
	...
	public String getUsername() {
		return user.getUsername(); // Trả về username của User
	}
	
}
```

### 5.2. Cấu hình `Spring Security` để sử dụng `CustomUserDetailsService`
Chúng ta cần cấu hình `Spring Security` để sử dụng `CustomUserDetailsService` thay vì `In-Memory` hoặc `Database` của chính ứng dụng.
...
@Configuration
public class SecurityConfig {
	private final CustomUserDetails customUserDetails; // Inject CustomUserDetails để xác thực User trong Database bên ngoài vừa tạo
	...
	} // Bean này trả về một đối tượng PasswordEncoder, dùng để mã hóa mật khẩu (password)

	@Bean // Bean này trả về một đối tượng AuthenticationManager, dùng để xác thực người dùng
	public AuthenticationManager authenticationManager(HttpSecurity http) throws Exception {
		AuthenticationManagerBuilder auth = http.getSharedObject(AuthenticationManagerBuilder.class); // Lấy ra một đối tượng AuthenticationManagerBuilder từ HttpSecurity 
		auth.userDetailsService(customUserDetails).passwordEncoder(passwordEncoder()); // Cấu hình AuthenticationManagerBuilder để sử dụng CustomUserDetails và giải mã mật khẩu bằng passwordEncoder rồi so sánh với mật khẩu người dùng nhập vào
		return auth.build(); // Trả về một đối tượng AuthenticationManager
	}
}
```

### 5.3. Login từ `Database` bên ngoài (MYSQL) và sử dụng `AuthenticationManager` để xác thực

>Congtroller.java
```java
...
	@PostMapping("/login")
	public ResponseEntity<?> auth(@RequestBody Login login) { // Phương thức xác thực User từ Database bên ngoài
		Authentication authentication = authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(login.getUsername(), login.getPassword())); // sau khi gọi phương thức này, sẽ gọi phương thức loadUserByUsername(username) của CustomUserDetails để xác thực User từ Database bên ngoài
		return ResponseEntity.ok("User authenticated successfully");
	}
```

## 6. Cấu hình CORS (Cross-Origin Resource Sharing) trong Spring Security
...
## 7. Cấu hình `SecurityFilterChain` trong `Spring Security`
...
- Khi đi vào một `route` nào đó, `Spring Security` sẽ sử dụng `SecurityFilterChain` để kiểm tra xem người dùng có quyền truy cập không theo các bước sau:
	1. Kiểm tra xem người dùng đã xác thực chưa. 
		```java
		        http.authorizeHttpRequests(authorizeRequests ->
					...
					/*
					 * Ngoài ra requestMatchers còn có thể kết hợp với các điều kiện khác như:
					 * .requestMatchers(PathRequest.toStaticResources().atCommonLocations()).permitAll() // Cho phép tất cả các request vào các tài nguyên tĩnh không cần xác thực
					 * .requestMatchers(...).hasAnyRole("ADMIN", "USER") // Chỉ cho phép các request có role ADMIN hoặc USER mới được truy cập
					 * .requestMatchers(...).hasAuthority("READ") // Chỉ cho phép các request có quyền READ mới được truy cập
					*/
	...
	3. Kiểm tra các chính sách bảo mật khác (như CSRF, XSS, Clickjacking, CORS...).
		```java
				...
				...
		```
	4. Thêm trưỡc, sau, hoặc thay thế các `Filter` mặc định của `Spring Security`.
		```java
				http.addFilterBefore(new CustomFilter(), BasicAuthenticationFilter.class); // Thêm một Filter trước BasicAuthenticationFilter
				http.addFilterAfter(new CustomFilter(), BasicAuthenticationFilter.class); // Thêm một Filter sau BasicAuthenticationFilter
				http.addFilterAt(new CustomFilter(), BasicAuthenticationFilter.class); // Thêm một Filter vào BasicAuthenticationFilter
		```
...
### 7.1. Cấu hình để `H2-DATABASE` hoạt động với Spring Security
...
```
## 8. Lấy thông tin người dùng hiện tại
Để lấy thông tin người dùng hiện tại, chúng ta sử dụng `SecurityContextHolder` và `Authentication`.

`SecurityContextHolder` > `AuthenticationManager` > `Authentication` > `Principal` > `UserDetails` > `User`.

> Sử dụng `SecurityContextHolder` để lấy thông tin người dùng hiện tại.
```java
...
> Sử dụng `Authentication` làm `params` của phương thức.
```java
....
    @GetMapping("/")
...
## 9. CSRF (Cross-Site Request Forgery) trong Spring Security
...
## 10. Sử dụng annotation `@Pre` và `@Post` trong Spring Security
`@PreAuthorize` và `@PostAuthorize` là 2 annotation trong `Spring Security` giúp chúng ta kiểm tra quyền truy cập của người dùng trước hoặc sau khi thực hiện một phương thức.

Để sử dụng `@PreAuthorize` và `@PostAuthorize`, chúng ta cần thêm annotation `@EnableMethodSecurity` vào class cấu hình `Spring Security`.
...
public class HomeController {
	...
	@GetMapping("/admin")
	@PreAuthorize("hasRole('ADMIN')") // Kiểm tra quyền truy cập của người dùng trước khi thực hiện phương thức
	public String admin() {
		return "Hello Admin";
	}

	@GetMapping("/{username}")
	@PreAuthorize("hasRole('USER') and #username = authentication.name") // Kiểm tra vai trò của người dùng và username truyền vào phải bằng với username của người dùng đã xác thực (login) để chỉ cho phép người dùng xem thông tin của mình
	@PostAuthorize("returnObject.username == authentication.name") // Kiểm tra sau khi thực hiện phương thức, username trả về phải bằng với username của người dùng đã xác thực (login) để chỉ cho phép người dùng xem thông tin của mình
...
## 11. Cấu hình `Spring Security` trong các ứng dụng `Reactive` - `WebFlux`
`WebFlux` là một framework mạnh mẽ và linh hoạt để xây dựng ứng dụng `Reactive` trong `Spring`. `WebFlux` hỗ trợ xây dựng ứng dụng `Reactive` với `Spring Boot`.

Đặc điểm của ứng dụng `Reactive`:
- `Non-Blocking`: Không chờ đợi kết quả trả về từ một `Thread` khác.
- `Asynchronous`: Thực hiện nhiều tác vụ cùng một lúc.
- `Event-Driven`: Sử dụng `Event-Driven` để xử lý các sự kiện.

Cấu hình `Spring Security` trong ứng dụng `Reactive` tươn tự như ứng dụng `Web MVC` hay `RestAPI` nhưng cần thêm chú thích Annotation `@EnableWebFluxSecurity` để cấu hình `Spring Security` trong ứng dụng `Reactive`.

Ngoài ra để chỉnh `Filter` trong ứng dụng `Reactive`, chúng ta sử dụng `SecurityWebFilterChain` thay vì `SecurityFilterChain`, `ServerHttpSecurity` thay vì `HttpSecurity`. Ví dụ cấu hình `Spring Security` trong ứng dụng `Reactive`:
...
import org.springframework.security.config.annotation.web.reactive.ServerHttpSecurity;
...
public class SecurityConfiguration {
	...
	public SecurityWebFilterChain securityWebFilterChain(ServerHttpSecurity http) {
		http.authorizeExchange(exchanges ->
			...
			exchanges.anyExchange().authenticated() // Tất cả các request khác đều cần phải xác thực mới được truy cập
		); // Cấu hình cho tất cả các request đều cần phải xác thực mới được truy cập
		...
		return http.build(); // Trả về một đối tượng SecurityWebFilterChain
	}
}
```	

## 12. Sử dụng `OncePerRequestFilter` trong `Spring Security`
`OncePerRequestFilter` là một `Filter` trong `Spring Security` giúp chúng ta thực hiện các tác vụ trước hoặc sau khi một request được xử lý.

Để sử dụng `OncePerRequestFilter`, chúng ta cần tạo một class kế thừa `OncePerRequestFilter` và override phương thức `doFilterInternal` để thực hiện các tác vụ trước hoặc sau khi một request được xử lý.

>filter/CustomFilter.java
```java
...
@Component // Đánh dấu đây là một Bean
@Order(1) // Xác định thứ tự của Filter, ví dụ có nhiều Filter, thì Filter có Order nhỏ hơn sẽ được thực hiện trước
public class CustomFilter extends OncePerRequestFilter { // Kế thừa OncePerRequestFilter để thực hiện các tác vụ trước hoặc sau khi một request được xử lý

	@Override
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException { // Phương thức thực hiện các tác vụ trước hoặc sau khi một request được xử lý (Trước khi vào Controller)
		System.out.println("Custom Filter"); // In 
		String xxx = request.getHeader("xxx"); // Lấy giá trị của header xxx từ request
		ì (xxx == null) {
			xxx = "default"; // Nếu header xxx không tồn tại, gán giá trị mặc định là default
		}
		response.addHeader("xxx", xxx); // Thêm một header xxx vào response với giá trị xxx 

		filterChain.doFilter(request, response); // Tiếp tục chuyển request và response cho các Filter tiếp theo
	}
}
```
