...
service:
 kafka:
  ...
  environment:
    ...
    - KAFKA_CFG_CONTROLLER_QUORUM_VOTERS=0@kafka:9093 # Quorum Voters của Kafka Server trong Cluster (0@kafka:9093)
    # Listeners
...
```
> LƯU Ý: CHÚNG TA CÓ THỂ TẠO NHIỀU `KAFKA BROKERS` BẰNG CÁCH THÊM CÁC `SERVICE` Y HỆT CHỈ THAY ĐỔI `NODE_ID`.
