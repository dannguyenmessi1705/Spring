...
> Lưu ý: Nếu muốn vừa nhận, vừa đẩy dữ liệu đi tiếp sang Kafka thì phải dùng `Function` (vì `Function` có tính chất là nhận vào và trả ra dữ liệu). Còn nếu chỉ muốn nhận dữ liệu từ Kafka thì dùng `Consumer` hoặc `Supplier`.
> Sau khi đẩy dữ liệu đi tiếp, nó sẽ tự động bindings với tên `<function>-out-0` (cần phải định nghĩa function cho hàm đó trước) và nếu muốn nhận dữ liệu từ Kafka thì phải tạo một `Consumer` hoặc `Supplier` khác để nhận dữ liệu từ Kafka.
