...
> Lưu ý: Nếu cấu hình `max-attempts` là `3` trong `Service` con, thì sẽ thực hiện tổng cộng chỉ `3 lần` so với `4 lần` trong `Gateway`. Vì `max-attempts` chỉ áp dụng cho `Service` con, còn `Gateway` sẽ thực hiện `1 lần` nữa trước khi gọi tới `Service` con.
