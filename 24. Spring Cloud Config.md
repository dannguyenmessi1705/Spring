...
## 4. Sử dụng `Actuator` để refresh cấu hình của `Service` con mỗi khi thay đổi cấu hình trên `Config Server`
...
### 4.1. Cấu hình `Actuator` cho `Service` con
...
### 4.2. Refresh cấu hình của `Service` con
