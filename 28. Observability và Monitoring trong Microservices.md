...
receivers:
  otlp:
    protocols:
      grpc: # Sử dụng giao thức gRPC để nhận dữ liệu từ các Microservices

processors:
  batch: # Sử dụng processor batch để xử lý dữ liệu trước khi gửi đi
    send_batch_size: 1024 # Kích thước batch gửi đi
    timeout: 10s # Thời gian chờ tối đa để gửi batch

exporters: 
  otlp/quickwit:
    endpoint: quickwit:7281 # Địa chỉ của Quickwit để gửi dữ liệu
    tls:
      insecure: true # Không sử dụng TLS để kết nối với Quickwit

service:
  pipelines: # Pipelines để xử lý dữ liệu
    logs: # Sử dụng pipeline logs để xử lý dữ liệu log
      receivers: [ otlp ]
      processors: [ batch ]
      exporters: [ otlp/quickwit ]
    metrics: # Sử dụng pipeline metrics để xử lý dữ liệu metrics (có thể bỏ, nếu sử dụng Prometheus)
      receivers: [ otlp ]
      processors: [ batch ]
      exporters: [ otlp/quickwit ]
    traces: # Sử dụng pipeline traces để xử lý dữ liệu traces (có thể bỏ, nếu sử dụng Prometheus)
      receivers: [ otlp ]
      processors: [ batch ]
      exporters: [ otlp/quickwit ]
```
...
services:
  otel-collector:
    ...
    ports:
      ...
      - "7281" # OTLP gRPC endpoint for QuickWit
...
Chúng ta cần cài đặt `Quickwit` để lưu trữ và tìm kiếm `Logs` của `Microservices`. Chúng ta sẽ sử dụng `Docker Compose` để cài đặt `Quickwit`:
```yml
services:
  quickwit:
    ...
    environment:
      ...
      QW_ENABLE_OPENTELEMETRY_OTLP_EXPORTER: true # Enable OpenTelemetry OTLP exporter
      OTEL_EXPORTER_OTLP_ENDPOINT: "http://localhost:7281" # Set OTLP exporter endpoint
```

### 5.5.4. Cấu hình `Grafana` để hiển thị `Logs` từ `Quickwit`
...
Chọn `Add data source` và chọn `Quickwit`, sau đó nhập các thông tin như url: `quickwit:7281/api/v1`, IndexId: `otel-traces-v0_7` giống với IndexId của Quickwit hiện ở màn frontend, body để hiện thị log: `body.message`.

### 5.5.5. Cấu hình `OpenTelemetry` để gửi `Logs` lên `Quickwit`
Chúng ta cần cấu hình `OpenTelemetry` để gửi `Logs` lên `Quickwit
`. Chúng ta sẽ sử dụng `OpenTelemetry` để gửi `Logs` lên `Quickwit`:
```yml
management:
  tracing:
    otel:
...
### 5.5.6. Cài đặt `Elasticsearch` và cấu hình `Quickwit` để lưu trữ `Logs`
...
```
### 5.5.7. Cấu hình `Grafana` để hiển thị `Logs` từ `Quickwit` và `Elasticsearch`
