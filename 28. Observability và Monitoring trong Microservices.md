...
### 5.2. Sử dụng thư viện `Micrometer-tracing-brdige-brave`.
...
Sau khi cài đặt thư vện, mặc định [%X{traceId}, %X{spanId}] sẽ được thêm vào `MDC` của `logs` để lưu trữ các thông tin của request.

#### 5.2.1.2. Cấu hình log pattern trong application.yml
Chúng ta có thể cấu hình `log pattern` trong `application.yml` theo ý muốn, ví dụ:
>application.yml
```yml
logging:
  pattern:
    console: "%d{HH:mm:ss.SSS} [%t] %-5level %logger{36} [${spring.application.name}, %X{traceId}, %X{spanId}] - %msg%n"
```
...
#### 5.2.1.4. Sử dụng ThreadContext để lưu trữ thông tin của request.
Chúng ta có thể sử dụng `ThreadContext` để lưu trữ thông tin của request, mục đích là để khi request qua các service khác, chúng ta vẫn có thể lấy được `traceId` và `spanId` của request.
```java
import brave.Span;
import brave.Tracer;
import lombok.extern.slf4j.Slf4j;
...
NGOÀI RA CHÚNG TA CÓ THỂ SỬ DỤNG THƯ VIỆN `spring-cloud-sleuth` ĐỂ TỰ ĐỘNG TẠO `TRACE-ID` VÀ `SPAN-ID` CHO CÁC REQUEST TRONG HỆ THỐNG TƯƠNG TỰ NHƯ `Micrometer-tracing-brdige-brave`.
