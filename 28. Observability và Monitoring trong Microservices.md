...
Chúng ta cần cấu hình `OpenTelemetry` để gửi `Logs` lên `Quickwit`. Chúng ta sẽ sử dụng `OpenTelemetry` để gửi `Logs` lên`Quickwit`, cách này không cần dependency `opentelemetry-javaagent`:
```application.properties
# Cấu hình OpenTelemetry để gửi Logs lên Quickwit
spring.application.name=LogQuickwit
otel.service.name=LogQuickwit
