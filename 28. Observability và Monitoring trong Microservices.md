...
## 5.5. Sử dụng `Quickwit` kết hợp với `OpenTelemetry` để lưu trữ và tìm kiếm `Logs` của `Microservices` thay thế cho `Loki` (SL4J2)
...
Đầu tiên với Cấu hình sử dụng `sl4j` chúng ta cần cấu hình file `log4j2.xml` để gửi `Logs`.
>log4j2.xml
```xml
<?xml version="1.0" encoding="UTF-8"?>
<Configuration>
    <Properties>
        <Property name="PATTERN">%d{yyyy-MM-dd'T'HH:mm:ss.SSSXXX} | %highlight{%-5level} | DIDAN | AppLog | Java |
            %cyan{%t} | ${project.name} | %blue{%X{correlationID}} | %magenta{%X{clientIP}} | %msg %n
        </Property>
        <property name="LOG_LEVEL">INFO</property>
        <property name="project.name">log-quickwit</property>
    </Properties>
    <Appenders>
        <!-- Console appender để hiển thị logs trên console -->
        <Console name="Console" target="SYSTEM_OUT">
            <PatternLayout pattern="${PATTERN}"/>
        </Console>
        <!-- JsonLayout để hiển thị logs dưới dạng JSON -->
        <Console name="JsonLayout" target="SYSTEM_OUT">
            <JsonTemplateLayout
                    charset="UTF-8"
                    locationInfoEnabled="true"
                    stackTraceEnabled="true"
                    eventTemplateUri="classpath:log4j2-json-template.json"
                    maxStringLength="4000"
                    truncatedStringSuffix=" ... (truncated)"
                    recyclerFactory="threadLocal">
                <EventTemplateAdditionalField key="service" value="log-quickwit"/>
                <EventTemplateAdditionalField key="environment" value="${env:ENVIRONMENT:-dev}"/>
            </JsonTemplateLayout>
        </Console>
        <!-- OpenTelemetryAppender để gửi logs lên Quickwit -->
        <OpenTelemetry name="OTLP"
                       ...
                       numLogsCapturedBeforeOtelInstall="2000"
        />

        <!-- RollingFile appender để ghi logs vào file và nén theo kích thước -->
        <RollingFile
                name="rollBySize"
                fileName="./logs/data.log"
                filePattern="./logs/archived/data.%i.log.gz"
    ...
    <Loggers>
        ...
        <Root level="${LOG_LEVEL}">
            ...
            <AppenderRef ref="Console"/>
            <AppenderRef ref="OTLP"/>
        </Root>
    </Loggers>
</Configuration>
```

Với `logback`, chúng ta cần cấu hình file `logback-spring.xml` để gửi `Logs` lên `Quickwit`:
>logback-spring.xml
```xml
<configuration>
...
services:
  accounts:
    ...
    environment:
      ...
      OTEL_METRIC_EXPORT_INTERVAL: 5000 # Tần suất gửi Metrics, 5000ms = 5s
      OTEL_TRACES_SAMPLER: "always_on" # Luôn gửi tất cả các Traces 
    ports:
