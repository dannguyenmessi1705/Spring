...
Cuối cùng, thêm profiles của `Tempo` vào `datasources` của Service Docker `Grafana` để hiển thị `Traces` của `Microservices` trong hệ thống: (Sử dụng `grafana` từ phần `#### 3.3.2`)
```yml
services:
...
  grafana: # Service Grafana dùng để hiển thị dữ liệu từ Loki và xử lý dữ liệu từ Alloy
