...
## 7. Sử dụng `Consul` để triển khai `Client-side Service Discovery` thay thế `Eureka`
Ngoài `Eureka`, chúng ta còn có thể sử dụng `Consul` để triển khai `Client-side Service Discovery`. `Consul` là một công cụ mã nguồn mở được phát triển bởi `HashiCorp`, nó cung cấp các tính năng như `service discovery`, `health checking`, `key/value store`, `multi-datacenter support`,...

Vì `Eureka` đã ngừng phát triển từ `Netflix`, nên hiện nay `Consul` đang được sử dụng phổ biến hơn trong các dự án `microservices`.

### 7.1. Cài đặt `Consul Server`
Cài đặt `Consul Server` bằng cách sử dụng `Docker`. Chạy lệnh sau để khởi động `Consul Server` trên `localhost:8500`
```yml
services:
  consult:
    image: consul:1.15.4
...
### 7.2. Các `dependency` cần thiết
Thêm `dependency` `Spring Cloud Consul` và nhơ phải thêm `Spring Boot Actuator` để cung cấp các endpoint để `Consul Server` kiểm tra sức khỏe của `service instances`
```xml
<dependency>
    <groupId>org.springframework.cloud</groupId>
...
### 7.3. Cấu hình `Consul Client` để đăng ký vào `Consul Server`
Đầu tiên thêm `@EnableDiscoveryClient` vào `Spring Boot Application` để kích hoạt `Consul Client`
```java
@SpringBootApplication
...
Thêm cấu hình `Consul Client` trong `application.yml`
```yml
server:
  port: 8080
...
Làm tương tự cho các `service` khác để đăng ký vào `Consul Server`.