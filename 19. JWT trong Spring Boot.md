...
@PostMapping("/login")
	public ResponseEntity<?> auth(@RequestBody Login login) {
		authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(login.getUsername(), login.getPassword())); // Xác thực người dùng từ database thay vì Basic Authentication (Sau khi đã Custom UserDetailsService)
		Authentication auth = SecurityContextHolder.getContext().getAuthentication(); // Lấy thông tin người dùng sau khi xác thực
