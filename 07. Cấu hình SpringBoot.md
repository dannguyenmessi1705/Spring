...
3. Chạy ứng dụng SpringBoot bằng lệnh: `java -jar target/tenProject.jar`.
    - Nếu muốn chạy ứng dụng SpringBoot với một cấu hình khác thủ công, chúng ta thêm tiền tố cấu hình vào sau `--` đặt ở cuối lệnh. Ví dụ: `java -jar target/tenProject.jar --spring.profiles.active=prod`. 
    - Hoặc có thể đê sau `-D` để cấu hình thông số khác. Ví dụ: `java -Dserver.port=8080 -jar target/tenProject.jar`.
    - Hoặc có thể dùng biến môi trường để cấu hình, với quy tắc `viết hoa` và `thay dấu '.' bằng '_'`. Ví dụ:
