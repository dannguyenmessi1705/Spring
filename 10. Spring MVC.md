...
```
### 3.1. Tạo file `.jspf` để chia nhỏ file `.jsp` thành các phần nhỏ hơn và include vào file `.jsp` chính, tránh việc lặp lại code.
...
>`hello.jsp`
```jsp
<%@ include file="header.jspf" %> <!-- Nhúng file header.jspf vào file hello.jsp -->
    <h1>Hello World!</h1>
<%@ include file="footer.jspf" %> <!-- Nhúng file footer.jspf vào file hello.jsp -->
```
...
## 8. SSE (Server-Sent Events) trong Spring MVC
`SSE (Server-Sent Events)` là một công nghệ cho phép server gửi dữ liệu đến client theo thời gian thực thông qua kết nối HTTP. Trong Spring MVC, chúng ta có thể sử dụng `SSE` để gửi dữ liệu từ server đến client một cách dễ dàng.

Ví dụ: Tạo một Controller sử dụng `SSE` để gửi dữ liệu đến client.
>`SseController.java`
```java
package com.example.demo;
import org.springframework.stereotype.Controller;
...
public class SseController {
    @GetMapping("/sse", produces = MediaType.TEXT_EVENT_STREAM_VALUE) // Chỉ định rằng phương thức này sẽ trả về dữ liệu dạng SSE
    public SseEmitter sse() {
        SseEmitter emitter = new SseEmitter();
        new Thread(() -> {
            try {
                for (int i = 0; i < 10; i++) {
                    emitter.send("SSE Message " + i); // Gửi dữ liệu đến client
                    Thread.sleep(1000); // Chờ 1 giây
                }
                emitter.complete(); // Kết thúc kết nối
            } catch (IOException | InterruptedException e) {
                emitter.completeWithError(e); // Xử lý lỗi
            }
        }).start();
        return emitter; // Trả về SseEmitter
    }
}
```