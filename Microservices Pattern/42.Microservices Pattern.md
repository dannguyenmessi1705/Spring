...
#### 5.4.2 Cấu hình Rate Limiter với Resilience4j
Cấu hình Rate Limiter trong `application.yml` để thiết lập các tham số như số
lượng yêu cầu tối đa và khoảng thời gian.
```yaml
resilience4j.ratelimiter:
  instances:
    externalApiRateLimiter: # Tên service rate limiter (có thể đặt tên tùy ý), nếu muốn áp dụng cho tất cả service thì bỏ trống (instances)
      limitForPeriod: 5 # Số yêu cầu tối đa trong một khoảng thời gian
      limitRefreshPeriod: 10s # Khoảng thời gian làm mới giới hạn
      timeoutDuration: 2s # Thời gian chờ khi vượt quá giới hạn
```

#### 5.4.3 Áp dụng Rate Limiter trong ExternalApiClient
Sử dụng annotation `@RateLimiter` trong `ExternalApiClient` để áp dụng Rate Limiter cho phương thức gọi dịch vụ bên ngoài.
```java
@Component
public class ExternalApiClient {
    private final WebClient webClient;
    public ExternalApiClient(WebClient.Builder webClientBuilder,
                             @Value("${rate_limiter.external_api.service}") String baseUrl) {
        this.webClient = webClientBuilder.baseUrl(baseUrl).build();
    }
    // Áp dụng Rate Limiter để giới hạn số lượng yêu cầu gửi đến dịch vụ bên ngoài, với fallback khi vượt quá giới hạn
    @RateLimiter(name = "externalApiRateLimiter", fallbackMethod = "fallbackExternalApi")
    public Mono<ExternalApiResponse> callExternalApi(String param) {
        return webClient.get()
                .uri("/external-endpoint?param={param}", param)
                .retrieve()
                .bodyToMono(ExternalApiResponse.class)
                .timeout(Duration.ofSeconds(3));
    }
    private Mono<ExternalApiResponse> fallbackExternalApi(String param, Throwable ex) {
        // Fallback trả về một response mặc định khi vượt quá giới hạn tốc độ
        return Mono.just(new ExternalApiResponse("default", "Fallback response due to rate limiting"));
    }
}
```
#### 5.4.4 Bố cục source code Rate Limiter
- `ProductionAggregateController` (`src/main/java/com/didan/pattern/microservices_pattern/rate_limiter/controller/ProductionAggregateController.java`): mở endpoint `GET /rate-limiter/product/{id}` để tổng hợp sản phẩm + đánh giá, và demo endpoint `GET /rate-limiter/calculator/{input}` có Rate Limiter + fallback.
- `ProductionAggregatorService` (`rate_limiter.service`): zip song song kết quả từ `ProductClient` và `ReviewClient`, map về `ProductAggregate`.
- `ProductClient` (`rate_limiter.client.ProductClient`): WebClient tới `rate_limiter.product.service`, `timeout(500ms)`, `onErrorResume(Mono.empty())` để fail-fast khi product không lấy được.
- `ReviewClient` (`rate_limiter.client.ReviewClient`): WebClient tới `rate_limiter.review.service`, gắn `@RateLimiter(name="review-service", fallbackMethod="fallback")` để hạn chế tốc độ call ra downstream; onStatus 4xx trả `Mono.empty()`, fallback trả danh sách rỗng khi bị limit.
- DTO: `Product`, `Review`, `ProductAggregate` nắm giữ dữ liệu trả về.

#### 5.4.5 Quy trình xử lý `GET /rate-limiter/product/{id}`
1) Controller nhận request → gọi `productionAggregatorService.aggregate(id)`.
2) Service log "Starting aggregation process..." rồi `Mono.zip(productClient.getProduct(id), reviewClient.getReviews(id))`.
   - Nhánh **ProductClient**: GET `{baseUrl}/{id}` với timeout 500ms; mọi lỗi/timeout → `Mono.empty()`.
   - Nhánh **ReviewClient**: GET `{baseUrl}/{id}`; nếu 4xx trả `Mono.empty()` (không error); nếu vượt limit của RateLimiter `review-service` → ném `RequestNotPermitted` và được bắt bởi fallback, trả `Collections.emptyList()`.
3) `Mono.zip` chỉ phát ra khi cả 2 Mono có giá trị; nếu product trống → zip hoàn thành rỗng → controller `defaultIfEmpty` trả 404.
4) Nếu product có và review bị limit/lỗi → `ProductAggregate` được tạo với `reviews=[]`, trả 200.
5) Nếu cả hai thành công → trả 200 với dữ liệu đầy đủ.

```mermaid
sequenceDiagram
Client->>ProductionAggregateController: GET /rate-limiter/product/{id}
ProductionAggregateController->>ProductionAggregatorService: aggregate(id)
par Parallel calls
ProductionAggregatorService->>ProductClient: GET /{id} (timeout 500ms)
ProductionAggregatorService->>ReviewClient: GET /{id} (RateLimiter name=review-service)
end
alt product lỗi/timeout
ProductClient-->>ProductionAggregatorService: Mono.empty()
ProductionAggregatorService-->>ProductionAggregateController: Mono.empty()
ProductionAggregateController-->>Client: 404 Not Found
else product ok, review bị limit/lỗi
ProductClient-->>ProductionAggregatorService: Product
ReviewClient-->>ProductionAggregatorService: [] (fallback)
ProductionAggregatorService-->>ProductionAggregateController: ProductAggregate(reviews=[])
ProductionAggregateController-->>Client: 200 OK
else cả hai ok
ProductionAggregatorService-->>ProductionAggregateController: ProductAggregate đầy đủ
...
#### 5.4.6 Chi tiết Rate Limiter cho ReviewClient
- Annotation `@RateLimiter(name = "review-service", fallbackMethod = "fallback")` chặn hoặc cho phép lời gọi WebClient tùy quota.
- Khi quota còn: request đi qua; nếu lỗi 4xx → `Mono.empty()` (không kích hoạt fallback); lỗi khác sẽ bong ra, được annotation chuyển thành `RequestNotPermitted` khi vượt limit.
- Khi vượt limit: fallback `fallback(Integer id, Throwable ex)` trả `Mono.just(Collections.emptyList())`, cho phép toàn bộ luồng aggregation tiếp tục với reviews rỗng thay vì văng lỗi 429 lên caller.
- Không có cấu hình `resilience4j.ratelimiter` trong `application.properties`; nếu cần tinh chỉnh (limitForPeriod, refreshPeriod, timeoutDuration) phải bổ sung vào cấu hình để tránh dùng default (50 req/period 500ns).

#### 5.4.7 Endpoint tính toán có Rate Limiter
- `doubleInput` (`GET /rate-limiter/calculator/{input}`) được gắn `@RateLimiter(name="calculator-service", fallbackMethod="fallback")` nhằm giới hạn tần suất xử lý CPU-bound demo.
- Khi được phép: trả ngay `input * 2`.
- Khi vượt ngưỡng: fallback trả `429 TOO_MANY_REQUESTS` cùng thông điệp từ exception (thường là `RequestNotPermitted`), tránh làm nghẽn thread và giúp client tự backoff.

#### 5.4.8 Lưu ý thiết kế
- **Fail-open cho dữ liệu bổ trợ**: Review bị limit → rỗng, còn product bị lỗi → trả 404 để tránh trả về aggregate thiếu sản phẩm.
- **Cần cấu hình ratelimiter rõ ràng**: thêm `resilience4j.ratelimiter.instances.review-service` và `calculator-service` để kiểm soát limit/refresh/timeout theo môi trường.
- **Giám sát**: log/metrics số lần fallback xảy ra giúp điều chỉnh limit hoặc mở rộng capacity downstream.
- **Kết hợp backoff**: client upstream nên backoff khi nhận 429 từ endpoint calculator; với review-service, fallback êm nên cần thêm metrics để không che giấu vấn đề lưu lượng.

#### 5.4.9 Cấu hình Rate Limiter trong `application.properties`
- File hiện tại chỉ khai báo base URL:
  - `rate_limiter.product.service=http://localhost:7070/sec09/product/`
  - `rate_limiter.review.service=http://localhost:7070/sec09/review/`
- Cần bổ sung cấu hình Resilience4j để các annotation `@RateLimiter(name="review-service")` và `@RateLimiter(name="calculator-service")` dùng đúng ngưỡng giới hạn, ví dụ:
**Ví dụ cấu hình & luồng xử lý**
```properties
# Downstream review: hạn chế 5 req/giây, fail-fast
resilience4j.ratelimiter.instances.review-service.limitForPeriod=5
resilience4j.ratelimiter.instances.review-service.limitRefreshPeriod=1s
resilience4j.ratelimiter.instances.review-service.timeoutDuration=0

# Endpoint calculator: cho 15 req/giây, fail-fast
resilience4j.ratelimiter.instances.calculator-service.limitForPeriod=15
resilience4j.ratelimiter.instances.calculator-service.limitRefreshPeriod=1s
resilience4j.ratelimiter.instances.calculator-service.timeoutDuration=0
```
**Chi tiết hành vi các tham số**:
- `limitForPeriod` + `limitRefreshPeriod`: Resilience4j cấp phát một “bucket” quota mới sau mỗi `limitRefreshPeriod`. Khi hết quota, lời gọi tiếp theo sẽ:
  - Ngay lập tức ném `RequestNotPermitted` nếu `timeoutDuration=0`.
  - Hoặc chờ tối đa `timeoutDuration` để chờ bucket mới nếu giá trị > 0 (phù hợp cho batch ít nhạy latency, không nên dùng với API realtime).
- `timeoutDuration`: với reactive WebFlux, giá trị > 0 làm pipeline chờ permit, có thể kéo dài latency tổng; để tránh treo, thường đặt 0 và xử lý fallback/429 ở caller.
- Mô hình cài đặt theo nhu cầu:
  - Downstream đắt tiền (review-service): đặt quota thấp, refresh ngắn (vd 5 req/1s) để bảo vệ API ngoài; fallback rỗng nên user vẫn nhận được product.
  - Endpoint CPU-bound (calculator-service): quota cao hơn (vd 10 req/1s) để demo; khi hết quota trả 429 từ fallback, buộc client backoff.
  - Nếu muốn ưu tiên người dùng trả phí: tách nhiều instance rate limiter theo header/tenant (vd `review-service-premium` 20 req/1s, `review-service-free` 5 req/1s) và áp dụng condition trong controller/client tương ứng.

- Với `GET /rate-limiter/product/{id}`:
  1) Request vào controller → gọi `ReviewClient.getReviews(id)` có `@RateLimiter(name="review-service")`.
  2) Bộ đếm `review-service` kiểm tra quota 5 req/giây:
     - Còn quota: cho phép, WebClient gọi downstream; 4xx → `Mono.empty()`, lỗi khác → truyền lên (nếu không bị limit).
     - Hết quota: ném `RequestNotPermitted`.
  3) Annotation bắt `RequestNotPermitted` và gọi `fallback(id, ex)` → trả `Collections.emptyList()`.
  4) `Mono.zip` tạo `ProductAggregate` với reviews rỗng và trả 200 (fail-open).
- Với `GET /rate-limiter/calculator/{input}`:
  1) Request vào controller → `@RateLimiter(name="calculator-service")`.
  2) Bộ đếm `calculator-service` kiểm tra quota 15 req/giây:
     - Còn quota: trả `input * 2` (200).
     - Hết quota: ném `RequestNotPermitted`.
  3) Fallback `fallback(input, ex)` trả `429 TOO_MANY_REQUESTS` kèm message lỗi, giúp client chủ động backoff.
- Trường hợp `timeoutDuration > 0` (ví dụ batch ít nhạy latency): với `timeoutDuration=2s`, nếu hết quota, call sẽ đợi tối đa 2 giây đến chu kỳ refresh tiếp theo để lấy permit; nếu vẫn không có, mới ném `RequestNotPermitted`. Điều này giảm tỷ lệ lỗi nhưng tăng độ trễ, không phù hợp cho API realtime.

### 5.4. Bulkhead Pattern
`Bulkhead Pattern` (nghĩa là "buồng kín") là một mẫu thiết kế trong kiến trúc phần mềm nhằm cách ly các thành phần hoặc dịch vụ khác nhau trong một hệ thống để ngăn chặn sự cố lan rộng. Tương tự như các buồng kín trên tàu thủy giúp ngăn nước tràn vào toàn bộ con tàu khi có sự cố, Bulkhead Pattern giúp giới hạn tác động của lỗi trong một phần của hệ thống, tránh ảnh hưởng đến toàn bộ hệ thống.

Ứng dụng của Bulkhead Pattern bao gồm:
- Cách ly các dịch vụ hoặc thành phần khác nhau trong một hệ thống để ngăn chặn sự cố lan rộng.
- Giúp duy trì tính ổn định và khả năng phục hồi của hệ thống khi một phần gặp sự cố.
- Cải thiện hiệu suất của hệ thống bằng cách giới hạn tài nguyên sử dụng cho từng phần của hệ thống.
- Hỗ trợ quản lý tài nguyên hiệu quả hơn trong các hệ thống phân tán.
- Tăng cường khả năng chịu lỗi của hệ thống bằng cách giới hạn tác động của các lỗi.

#### 5.4.1 Cấu hình Bulkhead với Resilience4j
Cấu hình Bulkhead trong `application.yml` để thiết lập các tham số như số lượng kết nối tối đa và thời gian chờ.
```yaml
resilience4j.bulkhead:
    instances:
        reviewServiceBulkhead: # Tên instance bulkhead (có thể đặt tên tùy ý)
            maxConcurrentCalls: 10 # Số cuộc gọi đồng thời tối đa
            maxWaitDuration: 0 # Thời gian chờ khi đạt đến giới hạn
```

#### 5.4.2 Áp dụng Bulkhead trong ReviewClient
Sử dụng annotation `@Bulkhead` trong `ReviewClient` để áp dụng Bulkhead cho phương thức gọi dịch vụ review.
```java
@Component
public class ReviewClient {
    private final WebClient webClient;
    public ReviewClient(WebClient.Builder webClientBuilder,
                        @Value("${bulkhead.review.service}") String baseUrl) {
        this.webClient = webClientBuilder.baseUrl(baseUrl).build();
    }
    @Bulkhead(name = "reviewServiceBulkhead", fallbackMethod = "fallbackReviews")
    public Mono<List<Review>> getReviews(String productId) {
        return webClient.get()
                .uri("/{id}/reviews", productId)
                .retrieve()
    ...
    private Mono<List<Review>> fallbackReviews(String productId, Throwable ex) {
        // Fallback trả về danh sách rỗng khi Bulkhead bị giới hạn hoặc có
        return Mono.just(Collections.emptyList());
    }
}
```

#### 5.4.3 Cấu trúc các thành phần và luồng hoạt động
- **Controller**: `ProductionAggregateController` mở endpoint `GET /bulkhead/product/{id}`, trả `Mono<ResponseEntity<ProductAggregate>>` với fallback 404 khi aggregation rỗng.
- **Aggregator Service**: `ProductionAggregatorService.aggregate(id)` zip song song `productClient.getProduct(id)` + `reviewClient.getReviews(id)`; nếu product trả `Mono.empty()` → `Mono.zip` rỗng, controller trả 404; nếu review fallback rỗng thì vẫn trả `ProductAggregate` với review rỗng.
- **ProductClient**: WebClient baseUrl `bulkhead.product.service`, timeout 500ms, `onErrorResume(Mono.empty())`; đóng vai trò “trip” luồng nếu sản phẩm không lấy được.
- **ReviewClient**:
  - Đánh dấu `@Bulkhead(name = "reviewServiceBulkhead", fallbackMethod = "fallbackReviews")`.
  - Pipeline: GET → `onStatus(4xx -> Mono.empty())` → `bodyToFlux` → `collectList` → `timeout(500ms)`.
  - Fallback `fallbackReviews` trả `Mono.just(Collections.emptyList())` khi bulkhead bị giới hạn hoặc khi timeout/lỗi.
- **BulkheadConfig**: bean `BulkheadConfigCustomizer` đặt `maxConcurrentCalls(10)` cho instance `reviewServiceBulkhead` (cho phép tối đa 10 cuộc gọi đồng thời).

```mermaid
sequenceDiagram
Client->>ProductionAggregateController: GET /bulkhead/product/{id}
ProductionAggregateController->>ProductionAggregatorService: aggregate(id)
par Song song
ProductionAggregatorService->>ProductClient: GET /{id} (timeout 500ms)
ProductionAggregatorService->>ReviewClient: GET /{id} (Bulkhead name=review-service)
end
alt product lỗi/timeout
ProductClient-->>ProductionAggregatorService: Mono.empty()
ProductionAggregatorService-->>ProductionAggregateController: Mono.empty()
ProductionAggregateController-->>Client: 404 Not Found
else product ok, review bị giới hạn bulkhead/lỗi
ProductClient-->>ProductionAggregatorService: Product
ReviewClient-->>ProductionAggregatorService: [] (fallback/bulkhead)
ProductionAggregatorService-->>ProductionAggregateController: ProductAggregate(reviews=[])
ProductionAggregateController-->>Client: 200 OK 
else cả hai ok
ProductionAggregatorService-->>ProductionAggregateController: ProductAggregate đầy đủ
...
**Lưu ý thiết kế**:
- Bulkhead gắn ở review (dữ liệu bổ trợ) nên fallback rỗng, đảm bảo không đánh sập toàn bộ API.
- `maxConcurrentCalls(10)` cần điều chỉnh theo lưu lượng thực tế và SLA.
- Giám sát số lần fallback xảy ra giúp điều chỉnh cấu hình bulkhead hoặc mở rộng capacity downstream.

**Giải thích chi tiết về Bulkhead Pattern với Resilience4j trong Spring Boot WebFlux**:
- `@Bulkhead(name = "reviewServiceBulkhead", fallbackMethod = "fallbackReviews")`: Annotation này áp dụng Bulkhead Pattern cho phương thức `getReviews` trong `ReviewClient`. Khi số lượng cuộc gọi đồng thời vượt quá `maxConcurrentCalls` (10 trong cấu hình), các cuộc gọi tiếp theo sẽ bị chặn và chuyển sang phương thức fallback `fallbackReviews`.
- `fallbackReviews(String productId, Throwable ex)`: Phương thức này được gọi khi Bulkhead bị giới hạn hoặc khi có lỗi xảy ra trong quá trình gọi dịch vụ review. Nó trả về một danh sách rỗng, cho phép luồng xử lý tiếp tục mà không làm gián đoạn toàn bộ quá trình aggregation.
- `maxConcurrentCalls: 10`: Cấu hình này giới hạn số lượng cuộc gọi đồng thời đến dịch vụ review. Khi có hơn 10 cuộc gọi đồng thời, các cuộc gọi tiếp theo sẽ bị chặn và chuyển sang phương thức fallback.
- `maxWaitDuration: 0`: Cấu hình này xác định thời gian chờ khi đạt đến giới hạn cuộc gọi đồng thời. Với giá trị 0, các cuộc gọi sẽ bị chặn ngay lập tức khi vượt quá giới hạn mà không chờ đợi.
- `timeout(Duration.ofMillis(500))`: Thiết lập thời gian chờ cho cuộc gọi đến dịch vụ review. Nếu cuộc gọi không hoàn thành trong 500ms, nó sẽ bị hủy và chuyển sang phương thức fallback.
- `onStatus(HttpStatusCode::is4xxClientError, response -> Mono.empty())`: Xử lý các lỗi 4xx từ dịch vụ review bằng cách trả về `Mono.empty()`, tránh kích hoạt fallback và cho phép luồng xử lý tiếp tục.
- `Mono.zip(...)`: Trong `ProductionAggregatorService`, phương thức này kết hợp kết quả từ `ProductClient` và `ReviewClient`. Nếu `ProductClient` trả về `Mono.empty()`, toàn bộ kết quả sẽ là rỗng và controller sẽ trả về 404. Nếu `ReviewClient` bị giới hạn bulkhead hoặc lỗi, nó sẽ trả về danh sách rỗng, nhưng vẫn cho phép trả về `ProductAggregate` với sản phẩm và đánh giá rỗng.
- Giám sát và điều chỉnh: Việc theo dõi số lần fallback xảy ra sẽ giúp bạn hiểu rõ hơn về lưu lượng truy cập và hiệu suất của dịch vụ review, từ đó điều chỉnh cấu hình bulkhead hoặc mở rộng capacity downstream nếu cần thiết.
- Tổng thể, việc sử dụng Bulkhead Pattern với Resilience4j trong Spring Boot WebFlux giúp cải thiện độ ổn định và khả năng phục hồi của hệ thống bằng cách giới hạn tác động của các lỗi trong dịch vụ review, đồng thời đảm bảo rằng các phần khác của hệ thống vẫn hoạt động bình thường.
