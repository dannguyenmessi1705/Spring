package com.didan.testperformance.first.config.socket.handler;

import com.didan.testperformance.first.config.socket.connector.NettyConnector;
import com.didan.testperformance.first.config.socket.decoder.IsoMessageDecoder;
import com.didan.testperformance.first.constant.ISO8385Message;
import com.didan.testperformance.first.util.ConnectorUtils;
import com.didan.testperformance.first.util.DateUtil;
import com.didan.testperformance.first.util.LogUtils;
import com.didan.testperformance.first.util.MsgUtils;
...
public class IsoMessageHandler extends SimpleChannelInboundHandler<ISOMsg> {
  ...
  public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
    if (evt instanceof IdleStateEvent) {
      ...
      iso.set(ISO8385Message.TRANSMISSION_DATETIME, DateUtil.dateToString(transDate, "GMT", "MMddHHmmss")); // Đặt trường ngày giờ truyền
      ...
      iso.set(ISO8385Message.REFERENCE_NUMBER, DateUtil.dateToString(transDate, DateUtil.YYYYMMDDHHMMSSSSS)); // Đặt số tham chiếu 37
